...
const MainLayout = () => {
    ...
    return (
        <div className="flex min-h-screen bg-slate-50 text-slate-900">
            {/* Sidebar */}
            <aside className={`bg-[#0f172a] text-white transition-all duration-500 ease-in-out flex flex-col z-20 shadow-2xl ${isSidebarOpen ? 'w-72' : 'w-24'}`}>
                <div className="p-8 border-b border-white/5 flex items-center justify-between">
                    <div className={`flex items-center gap-4 transition-all duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>
                ...
                <nav className="flex-1 p-6 space-y-3 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="pt-4 pb-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Operations</div>
                    ...
                    <div className="pt-4 pb-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Directory</div>
                    ...
                    <div className="pt-4 pb-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Analytics</div>
                    <SidebarLink id="reports" icon="bar-chart-3" label="Business Intel" isOpen={isSidebarOpen} />

                    <div className="pt-4 pb-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Field Work</div>
                    <SidebarLink id="field_ops" icon="smartphone" label="Field Operations" isOpen={isSidebarOpen} />
                </nav>
            ...
            <main className="flex-1 flex flex-col min-h-screen overflow-hidden bg-[#f8fafc]">
                <header className="h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-4 sm:px-10 sticky top-0 z-10 shadow-sm">
                    <div className="flex flex-col min-w-0">
                        <h2 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight capitalize truncate max-w-[150px] sm:max-w-none">{activeTab.replace('_', ' ')}</h2>
                    ...
                    <div className="flex items-center gap-3 sm:gap-6">
                        <button className="relative text-slate-400 hover:text-slate-600 transition-colors">
                            <Icon name="bell" /><span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full border-2 border-white"></span>
                        </button>
                        ...
                        <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-slate-900 rounded-xl border-2 border-white shadow-lg flex items-center justify-center font-black text-brand-500 text-sm sm:text-base">
                                {user?.name?.charAt(0)}
                            </div>
                        </div>
                    </div>
